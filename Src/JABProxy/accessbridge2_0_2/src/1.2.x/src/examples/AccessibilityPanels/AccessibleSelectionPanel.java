...
/*
 * @(#)AccessibleSelectionPanel.java	1.8 02/01/17
 */
...
public class AccessibleSelectionPanel 
  extends AccessibilityPanel implements ActionListener {
	...
	if (ac == null) {
	    ...
	    if (as == null) {
		childrenList.add("(AccessibleSelection not supported)");
	    } else {
		Accessible child;
	...
	if (as != null) {
	    if (cmd == "Refresh") {
		; // dummy, just to cause updateSelectionInfo to be called
	    } else if (cmd == "Add") {
