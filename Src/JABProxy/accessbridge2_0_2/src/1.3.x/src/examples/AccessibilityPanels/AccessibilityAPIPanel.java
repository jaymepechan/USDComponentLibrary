...
/*
 * @(#)AccessibilityAPIPanel.java	1.29 02/01/17
 */
...
public class AccessibilityAPIPanel extends AccessibilityPanel {
	...
	if (ac == null) {
	    ...
	    if (ai != null && ai.length > 0) {
	        mouseInformation.add("interface AccessibleIcon:");
	        mouseInformation.add("    Desc:  " + ai[0].getAccessibleIconDescription());
	        mouseInformation.add("    Width:  " + ai[0].getAccessibleIconWidth());
	        mouseInformation.add("    Height:  " + ai[0].getAccessibleIconHeight());
	    }
	    ...
	    if (ars != null && ars.size() > 0) {
	        mouseInformation.add("interface AccessibleRelationSet:");
	        mouseInformation.add("    Relation(s): " + ac.getAccessibleRelationSet());
	        AccessibleRelation member = ars.get(AccessibleRelation.MEMBER_OF);
		if (member != null) {
		    Object [] target = member.getTarget();
		    String s = new String("    member of: [");
		    for (int i = 0; i < target.length; i++) {
		        if (target[i] instanceof Accessible) {
		            Accessible a = (Accessible)target[i];
			    AccessibleContext ac2 = a.getAccessibleContext();
			    if (ac2 != null) {
			        s = s + ac2.getAccessibleName();
	    ...
	    //
            // AccessibleTable implemented by current component
            //
            AccessibleTable table = ac.getAccessibleTable();
	    if (table != null) {
                mouseInformation.add("interface AccessibleTable:");
		mouseInformation.add("    Table size: [" + table.getAccessibleRowCount() + ", " + table.getAccessibleColumnCount() + "]");
	    }
	    
	    //
            // AccessibleTable implemented by the parent of the 
	    // current component
            //
	    Accessible tableParent = ac.getAccessibleParent();
	    if (tableParent != null) {
		AccessibleContext tableContext = tableParent.getAccessibleContext();
		if (tableContext != null) {
		    table = tableContext.getAccessibleTable();
		    if (table != null) {
			mouseInformation.add("interface AccessibleTable (from table cell parent):");
			mouseInformation.add("    Table size: [" + table.getAccessibleRowCount() + ", " + table.getAccessibleColumnCount() + "]");
			int index = ac.getAccessibleIndexInParent();
			int row = index / table.getAccessibleColumnCount();
			int column = index % table.getAccessibleColumnCount();
			mouseInformation.add("    Cell location: [" + row + ", " + column + "]");
			mouseInformation.add("    Cell row extent: "+table.getAccessibleRowExtentAt(row, column));
			mouseInformation.add("    Cell column extent: "+table.getAccessibleColumnExtentAt(row, column));
			AccessibleTable headerTable = table.getAccessibleColumnHeader();
			if (headerTable != null) {
			    Accessible header = headerTable.getAccessibleAt(0, column);
			    if (header != null) {
				AccessibleContext headerContext = header.getAccessibleContext();
				if (headerContext != null) {
				    mouseInformation.add("    Cell header: "+headerContext.getAccessibleName());
				}
			    }
			}
		    }
