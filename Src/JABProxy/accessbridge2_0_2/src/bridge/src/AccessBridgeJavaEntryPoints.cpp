...
/*
 * @(#)AccessBridgeJavaEntryPoints.cpp	1.39 05/05/03
 */
...
#define FIND_CLASS(classRef, className)						\
	...
	if (localClassRef == (jclass) 0) {					\
		...
		PrintDebugString("    -> jniEnv = %X", jniEnv);			\
		return FALSE;							\
	}									\
...
#define FIND_METHOD(methodID, classRef, methodString, methodSignature);			 \
	...
	if (methodID == (jmethodID) 0) {						 \
		...
		PrintDebugString("    -> jniEnv = %X; classRef = %X", jniEnv, classRef); \
		return FALSE;								 \
	}
...
AccessBridgeJavaEntryPoints::BuildJavaEntryPoints() {
    ...
    FIND_CLASS(rectangleClass, "java/awt/Rectangle");
    FIND_CLASS(accessibleTextClass, "javax/accessibility/AccessibleText")
...
AccessBridgeJavaEntryPoints::isSameObject(jobject obj1, jobject obj2) {
    ...
    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::isSameObject(%X %X):", obj1, obj2);

    returnVal = (BOOL) jniEnv->IsSameObject(obj1, obj2);
...
AccessBridgeJavaEntryPoints::getAccessibleContextFromHWND(jint window) {
    ...
    if (getAccessibleContextFromHWNDMethod != (jmethodID) 0) {
        returnedAccessibleContext = jniEnv->CallObjectMethod(accessBridgeObject, getAccessibleContextFromHWNDMethod, 
                                                             window);
 ...
 */
BOOL
AccessBridgeJavaEntryPoints::setTextContents(const AccessibleContext accessibleContext, const wchar_t *text) {
    jthrowable exception;
    BOOL result = FALSE;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::setTextContents(%X. %ls):", 
                     accessibleContext, text);

    if (setTextContentsMethod != (jmethodID) 0) {

        // create a Java String for the text
        jstring textString = jniEnv->NewString(text, wcslen(text));
        if (textString == 0) {
 ...
 */
AccessibleContext
AccessBridgeJavaEntryPoints::getParentWithRole(AccessibleContext accessibleContext, const wchar_t *role) {
    jthrowable exception;
    AccessibleContext rAccessibleContext;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::getParentWithRole(%X):", 
                     accessibleContext);

    if (getParentWithRoleMethod != (jmethodID) 0) {

        // create a Java String for the role
        jstring roleName = jniEnv->NewString(role, wcslen(role));
        if (roleName == 0) {
            PrintDebugString("\r    NewString failed");
            return FALSE;
        }

        rAccessibleContext = (AccessibleContext)jniEnv->CallObjectMethod(accessBridgeObject, 
                                                                         getParentWithRoleMethod, 
                                                                         accessibleContext, roleName);
        EXCEPTION_CHECK("Getting PageTab - call to CallObjectMethod()", (AccessibleContext)0);
        PrintDebugString("\r\n    rAccessibleContext = %x", rAccessibleContext);
        return rAccessibleContext;
    } else {
 ...
 */
AccessibleContext
AccessBridgeJavaEntryPoints::getTopLevelObject(AccessibleContext accessibleContext) {
    jthrowable exception;
    AccessibleContext rAccessibleContext;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::getTopLevelObject(%X):", 
                     accessibleContext);
    ...
    if (getTopLevelObjectMethod != (jmethodID) 0) {
        rAccessibleContext = (AccessibleContext)jniEnv->CallObjectMethod(accessBridgeObject, 
                                                                         ...
                                                                         accessibleContext);
        EXCEPTION_CHECK("Getting TopLevelObject - call to CallObjectMethod()", (AccessibleContext)0);
        PrintDebugString("\r\n    rAccessibleContext = %x", rAccessibleContext);
        return rAccessibleContext;
    } else {
 ...
 */
AccessibleContext
AccessBridgeJavaEntryPoints::getParentWithRoleElseRoot(AccessibleContext accessibleContext, const wchar_t *role) {
    jthrowable exception;
    AccessibleContext rAccessibleContext;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::getParentWithRoleElseRoot(%X):", 
                     accessibleContext);
    ...
    if (getParentWithRoleElseRootMethod != (jmethodID) 0) {
        ...
        rAccessibleContext = (AccessibleContext)jniEnv->CallObjectMethod(accessBridgeObject, 
                                                                         ...
                                                                         accessibleContext, roleName);
        EXCEPTION_CHECK("Getting RealObject - call to CallObjectMethod()", (AccessibleContext)0);
        PrintDebugString("\r\n    rAccessibleContext = %x", rAccessibleContext);
        return rAccessibleContext;
    } else {
 ...
 */
jint 
AccessBridgeJavaEntryPoints::getObjectDepth(AccessibleContext accessibleContext) {
    jthrowable exception;
    jint rResult;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::getObjectDepth(%X):", 
                     accessibleContext);
 ...
 */
AccessibleContext
AccessBridgeJavaEntryPoints::getActiveDescendent(AccessibleContext accessibleContext) {
    jthrowable exception;
    AccessibleContext rAccessibleContext;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::getActiveDescendent(%X):", 
                     accessibleContext);
 ...
 */
BOOL 
AccessBridgeJavaEntryPoints::requestFocus(AccessibleContext accessibleContext) {

    jthrowable exception;
    BOOL result = FALSE;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::requestFocus(%X):", 
                     accessibleContext);
 ...
 */
BOOL 
AccessBridgeJavaEntryPoints::selectTextRange(AccessibleContext accessibleContext, int startIndex, int endIndex) {

    jthrowable exception;
    BOOL result = FALSE;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::selectTextRange(%X start = %d end = %d):", 
...
/* NEW FASTER CODE!!*/
BOOL 
AccessBridgeJavaEntryPoints::getTextAttributesInRange(AccessibleContext accessibleContext, 
    ...
    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::selectTextAttributes(%X start = %d end = %d):",
                     accessibleContext, startIndex, endIndex);
    
    *len = 0;
    result = getAccessibleTextAttributes(accessibleContext, startIndex, attributes);
    if (result != TRUE) {
        return FALSE;
    }
    (*len)++;
	...
	AccessibleTextAttributesInfo test_attributes = *attributes;
        ...
        if (getAccessibleAttributesAtIndexFromContextMethod != (jmethodID) 0) {
            ...
            PrintDebugString("  returned from CallObjectMethod(), js = %X", js);
            if (js != (jstring) 0) {
                stringBytes = (const wchar_t *) jniEnv->GetStringChars(js, 0);
 ...
 */
int
AccessBridgeJavaEntryPoints::getVisibleChildrenCount(const AccessibleContext accessibleContext) {
    
    jthrowable exception;
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getVisibleChildrenCount(%X)",
                     accessibleContext);
 ...
 * Bug ID 4944762- getVisibleChildren for list-like components needed
 */
BOOL AccessBridgeJavaEntryPoints::getVisibleChildren(const AccessibleContext accessibleContext, 
                                                     const int nStartIndex, 
                                                     /* OUT */ VisibleChildrenInfo *visibleChildrenInfo) {
    
    jthrowable exception;

    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getVisibleChildren(%X, startIndex = %d)",
    ...
    for (int i = nStartIndex; (i < numChildren) && (i < nStartIndex + MAX_VISIBLE_CHILDREN); i++) {
        ...
        visibleChildrenInfo->children[bufIndex] = jniEnv->CallObjectMethod(accessBridgeObject,
                                                                           getVisibleChildMethod,
                                                                           accessibleContext, 
                                                                           i);
        EXCEPTION_CHECK("##### getVisibleChildMethod - call to CallObjectMethod()", FALSE);
        PrintDebugString("  ##### visible child = 0x%x", visibleChildrenInfo->children[bufIndex]);
 ...
 */
BOOL 
AccessBridgeJavaEntryPoints::setCaretPosition(AccessibleContext accessibleContext, int position) {

    jthrowable exception;
    BOOL result = FALSE;

    PrintDebugString("\r\nIn AccessBridgeJavaEntryPoints::setCaretPostion(%X position = %d):", 
...
AccessBridgeJavaEntryPoints::getVersionInfo(AccessBridgeVersionInfo *info) {
    ...
    if (getJavaVersionPropertyMethod != (jmethodID) 0) {
        js = (jstring) jniEnv->CallObjectMethod(accessBridgeObject,
                                                getJavaVersionPropertyMethod);
        EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to CallObjectMethod()", FALSE);
        PrintDebugString("  returned from CallObjectMethod(), js = %X", js);
        if (js != (jstring) 0) {
            stringBytes = (const wchar_t *) jniEnv->GetStringChars(js, 0);
            EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to GetStringChars()", FALSE);
            ...
            EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to GetStringLength()", FALSE);
            jniEnv->ReleaseStringChars(js, stringBytes);
            EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to ReleaseStringChars()", FALSE);
            jniEnv->CallVoidMethod(accessBridgeObject, 
                                   decrementReferenceMethod, js);
            EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to CallVoidMethod()", FALSE);
            wPrintDebugString(L"  Access Bridge version = %ls", info->bridgeJavaClassVersion);
            jniEnv->DeleteLocalRef(js);
            EXCEPTION_CHECK("Getting AccessBridgeVersionProperty - call to DeleteLocalRef()", FALSE);
            PrintDebugString("  Finished call to DeleteLocalRef()");
        } else {
            PrintDebugString("  Access Bridge version is null.");
            info->bridgeJavaClassVersion[0] = (wchar_t) 0;
            return FALSE;
        }
    } else {
        PrintDebugString("  Error! either env == 0 or getVersionInfoMethod == 0");
        return FALSE;
    }
...
BOOL AccessBridgeJavaEntryPoints::verifyAccessibleText(jobject obj) {
    ...
    if (getAccessibleTextFromContextMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  AccessibleText = %X", returnedJobject);
...
AccessBridgeJavaEntryPoints::getAccessibleContextAt(jint x, jint y, jobject AccessibleContext) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleContextAt(%d, %d, %X):",
    ...
    if (getAccessibleContextAtMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  Returning - returnedAccessibleContext = %X; globalRef = %X", 
...
AccessBridgeJavaEntryPoints::getAccessibleContextInfo(jobject AccessibleContext, AccessibleContextInfo *info) {
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleContextInfo(%X):", AccessibleContext);
    ...
    PrintDebugString("*** jniEnv: %X; accessBridgeObject: %X; AccessibleContext: %X ***", 
    ...
    if (getAccessibleComponentFromContextMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  AccessibleComponent = %X", returnedJobject);
    ...
    if (getAccessibleActionFromContextMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  AccessibleAction = %X", returnedJobject);
    ...
    if (getAccessibleSelectionFromContextMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  AccessibleSelection = %X", returnedJobject);
    ...
    if (getAccessibleValueFromContextMethod != (jmethodID) 0) {
        ...
        PrintDebugString("  AccessibleValue = %X", returnedJobject);
        if (returnedJobject != (jobject) 0) {
    ...
    if (getAccessibleHypertextMethod != (jmethodID) 0 &&
        ...
        PrintDebugString("  AccessibleHypertext = %X",
                         returnedJobject);
        if (returnedJobject != (jobject) 0) {
...
AccessBridgeJavaEntryPoints::getAccessibleChildFromContext(jobject AccessibleContext, jint childIndex) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleChildContext(%x, %d):", 
...
AccessBridgeJavaEntryPoints::getAccessibleParentFromContext(jobject AccessibleContext)
{
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleParentFromContext(%x):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleTableInfo(jobject accessibleContext, 
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleTableInfo(%X):", 
                     accessibleContext);
    ...
    if (getAccessibleTableFromContextMethod != (jmethodID) 0) {
        PrintDebugString("##### Calling getAccessibleTableFromContextMethod ...");
        tableInfo->accessibleTable = jniEnv->CallObjectMethod(accessBridgeObject, 
                                                              getAccessibleTableFromContextMethod, 
                                                              accessibleContext);
        PrintDebugString("##### ... Returned from getAccessibleTableFromContextMethod");
        EXCEPTION_CHECK("##### Getting AccessibleTable - call to CallObjectMethod()", FALSE);
        PrintDebugString("  ##### accessibleTable = %X", tableInfo->accessibleTable);
    } else {
        PrintDebugString("  ##### Error! either env == 0 or getAccessibleTableFromContextMethod == 0");
        return FALSE;
    }
    ...
    if (getContextFromAccessibleTableMethod != (jmethodID) 0) {
        ...
        tableInfo->accessibleContext = jniEnv->CallObjectMethod(accessBridgeObject, 
                                                                getContextFromAccessibleTableMethod, 
                                                                accessibleContext);
        PrintDebugString("##### ... Returned from getContextFromAccessibleTable Method");
        EXCEPTION_CHECK("##### Getting AccessibleTable - call to CallObjectMethod()", FALSE);
        PrintDebugString("  ##### accessibleContext = %X", tableInfo->accessibleContext);
    } else {
...
AccessBridgeJavaEntryPoints::getAccessibleTableCellInfo(jobject accessibleTable, jint row, jint column,
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleTableCellInfo(%X): row=%d, column=%d", 
    ...
    if (getAccessibleTableCellAccessibleContextMethod != (jmethodID) 0) {
        tableCellInfo->accessibleContext = jniEnv->CallObjectMethod(accessBridgeObject,
        ...
        PrintDebugString("  ##### table AccessibleContext = %x", tableCellInfo->accessibleContext);
    } else {
...
AccessBridgeJavaEntryPoints::getAccessibleTableRowHeader(jobject acParent, AccessibleTableInfo *tableInfo) {
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleTableRowHeader(%X):", 
                     acParent);
...
AccessBridgeJavaEntryPoints::getAccessibleTableColumnHeader(jobject acParent, AccessibleTableInfo *tableInfo) {
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleTableColumnHeader(%X):", 
                     acParent);
...
AccessBridgeJavaEntryPoints::getAccessibleTableRowDescription(jobject acParent, jint row) {
    ...
    PrintDebugString("\r\n##### Calling AccessBridgeJavaEntryPoints::getAccessibleTableRowDescription(%X):", 
                     acParent);
...
AccessBridgeJavaEntryPoints::getAccessibleTableRowSelectionCount(jobject accessibleTable) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableRowSelectionCount(%X)", 
                     accessibleTable);
...
AccessBridgeJavaEntryPoints::isAccessibleTableRowSelected(jobject accessibleTable, jint row) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::isAccessibleTableRowSelected(%X, %d)", 
                     accessibleTable, row);
...
AccessBridgeJavaEntryPoints::getAccessibleTableRowSelections(jobject accessibleTable, jint count, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableRowSelections(%X, %d %X)", 
...
AccessBridgeJavaEntryPoints::getAccessibleTableColumnSelectionCount(jobject accessibleTable) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableColumnSelectionCount(%X)", 
                     accessibleTable);
...
AccessBridgeJavaEntryPoints::isAccessibleTableColumnSelected(jobject accessibleTable, jint column) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::isAccessibleTableColumnSelected(%X, %d)", 
...
AccessBridgeJavaEntryPoints::getAccessibleTableColumnSelections(jobject accessibleTable, jint count, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableColumnSelections(%X, %d, %X)", 
                     accessibleTable, count, selections);
...
AccessBridgeJavaEntryPoints::getAccessibleTableRow(jobject accessibleTable, jint index) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableRow(%X, index=%d)", 
    ...
    if (getAccessibleTableRowMethod != (jmethodID) 0) {
        result = jniEnv->CallIntMethod(accessBridgeObject,
                                       ...
                                       accessibleTable, index);
        EXCEPTION_CHECK("##### Getting getAccessibleTableRow - call to CallIntMethod()", FALSE);
...
AccessBridgeJavaEntryPoints::getAccessibleTableColumn(jobject accessibleTable, jint index) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableColumn(%X, index=%d)", 
                     accessibleTable, index);
    ...
    if (getAccessibleTableColumnMethod != (jmethodID) 0) {
        result = jniEnv->CallIntMethod(accessBridgeObject,
                                       ...
                                       accessibleTable, index);
        EXCEPTION_CHECK("##### Getting getAccessibleTableColumn - call to CallIntMethod()", FALSE);
...
AccessBridgeJavaEntryPoints::getAccessibleTableIndex(jobject accessibleTable, jint row, jint column) {
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleTableIndex(%X, row=%d, col=%d)", 
                     accessibleTable, row, column);
...
AccessBridgeJavaEntryPoints::getAccessibleRelationSet(jobject accessibleContext, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleRelationSet(%X, %X)", 
    ...
    for (int i = 0; i < relationSet->relationCount && i < MAX_RELATIONS; i++) {
        ...
        for (int j = 0; j < relationSet->relations[i].targetCount && j < MAX_RELATION_TARGETS; j++) {
            relationSet->relations[i].targets[j] = jniEnv->CallObjectMethod(accessBridgeObject,
...
AccessBridgeJavaEntryPoints::getAccessibleHypertext(jobject accessibleContext, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleHypertext(%X, %X)", 
                     accessibleContext, hypertext);

    // get the AccessibleHypertext
    hypertext->accessibleHypertext = jniEnv->CallObjectMethod(accessBridgeObject,
                                                              getAccessibleHypertextMethod,
                                                              accessibleContext);
    EXCEPTION_CHECK("##### Getting AccessibleHypertext - call to CallObjectMethod()", FALSE);
    PrintDebugString("  ##### AccessibleHypertext = %x", hypertext->accessibleHypertext);
    ...
    for (int i = 0; i < hypertext->linkCount && i < MAX_HYPERLINKS; i++) {
        ...
        // get the hyperlink
        hypertext->links[i].accessibleHyperlink = jniEnv->CallObjectMethod(accessBridgeObject,
                                                                           getAccessibleHyperlinkMethod,
                                                                           accessibleContext,
                                                                           i);
        EXCEPTION_CHECK("##### Getting AccessibleHyperlink - call to CallObjectMethod()", FALSE);
        PrintDebugString("  ##### AccessibleHyperlink = %x", hypertext->links[i].accessibleHyperlink);
...
AccessBridgeJavaEntryPoints::activateAccessibleHyperlink(jobject accessibleContext, 
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::activateAccessibleHyperlink(%X, %X):",
    ...
    if (activateAccessibleHyperlinkMethod != (jmethodID) 0) {
        ...
        PrintDebugString("\r\n  Error! either jniEnv == 0 or activateAccessibleMethod == 0");
        return FALSE;
    }
}
 ...
 */
BOOL 
AccessBridgeJavaEntryPoints::getAccessibleHypertextExt(const AccessibleContext accessibleContext,
    ...
    const wchar_t *stringBytes;
    jsize length;
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleHypertextExt(%X, %X, startIndex = %d)",
                     accessibleContext, hypertext, nStartIndex);
    
    // get the AccessibleHypertext
    hypertext->accessibleHypertext = jniEnv->CallObjectMethod(accessBridgeObject, getAccessibleHypertextMethod,
                                                              accessibleContext);
    EXCEPTION_CHECK("##### Getting AccessibleHypertext - call to CallObjectMethod()", FALSE);
    PrintDebugString("  ##### AccessibleHypertext = %x", hypertext->accessibleHypertext);
    ...
    for (int i = nStartIndex; (i < hypertext->linkCount) && (i < nStartIndex + MAX_HYPERLINKS); i++) {
        ...
        // get the hyperlink
        hypertext->links[bufIndex].accessibleHyperlink = jniEnv->CallObjectMethod(accessBridgeObject,
                                                                                  getAccessibleHyperlinkMethod,
                                                                                  hypertext->accessibleHypertext, 
                                                                                  i);
        EXCEPTION_CHECK("##### Getting AccessibleHyperlink - call to CallObjectMethod()", FALSE);
        PrintDebugString("  ##### AccessibleHyperlink = %x", hypertext->links[bufIndex].accessibleHyperlink);
...
jint AccessBridgeJavaEntryPoints::getAccessibleHyperlinkCount(const AccessibleContext accessibleContext) {
...
jint AccessBridgeJavaEntryPoints::getAccessibleHypertextLinkIndex(const AccessibleHypertext hypertext,
                                                                  const jint nIndex) {
    
    jthrowable exception;
    
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleHypertextLinkIndex(%X, index = %d)",
                     hypertext, nIndex);
...
BOOL AccessBridgeJavaEntryPoints::getAccessibleHyperlink(const AccessibleHypertext hypertext,
                                                         const jint index, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleHyperlink(%X, index = %d)",
                     hypertext, index);
    
    
    // get the hyperlink
    info->accessibleHyperlink = jniEnv->CallObjectMethod(accessBridgeObject,
                                                         getAccessibleHyperlinkMethod,
                                                         hypertext, 
                                                         index);
    EXCEPTION_CHECK("##### Getting AccessibleHyperlink - call to CallObjectMethod()", FALSE);
    PrintDebugString("  ##### AccessibleHyperlink = %x", info->accessibleHyperlink);
...
BOOL AccessBridgeJavaEntryPoints::getAccessibleKeyBindings(jobject accessibleContext, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleKeyBindings(%X, %X)", 
...
BOOL AccessBridgeJavaEntryPoints::getAccessibleIcons(jobject accessibleContext, 
    ...
    PrintDebugString("\r\n##### AccessBridgeJavaEntryPoints::getAccessibleIcons(%X, %X)", 
...
BOOL AccessBridgeJavaEntryPoints::doAccessibleActions(jobject accessibleContext, 
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::doAccessibleActions(%X, #actions %d %s):",
                     accessibleContext, 
    ...
    for (int i = 0; i < actionsToDo->actionsCount && i < MAX_ACTIONS_TO_DO; i++) {
        ...
        jstring javaName = jniEnv->NewString(actionName, wcslen(actionName));
        if (javaName == 0) {
            PrintDebugString("\r    NewString failed");
            *failure = i;
            return FALSE;
...
AccessBridgeJavaEntryPoints::getAccessibleTextInfo(jobject AccessibleContext,
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextInfo(%X, %d, %d):", 
...
AccessBridgeJavaEntryPoints::getAccessibleTextItems(jobject AccessibleContext,
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextItems(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleTextSelectionInfo(jobject AccessibleContext, 
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextSelectionInfo(%X):", 
                     AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleTextAttributes(jobject AccessibleContext, jint index, AccessibleTextAttributesInfo *attributes) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextAttributes(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleTextRect(jobject AccessibleContext, AccessibleTextRectInfo *rectInfo, jint index) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextRect(%X), index = %d", 
                     AccessibleContext, index);
...
AccessBridgeJavaEntryPoints::getCaretLocation(jobject AccessibleContext, AccessibleTextRectInfo *rectInfo, jint index) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getCaretLocation(%X), index = %d", 
                     AccessibleContext, index);
...
AccessBridgeJavaEntryPoints::getAccessibleTextLineBounds(jobject AccessibleContext, jint index, jint *startIndex, jint *endIndex) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextLineBounds(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleTextRange(jobject AccessibleContext, 
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleTextRange(%X, %d, %d, *text, %d):", AccessibleContext, start, end, len);
...
AccessBridgeJavaEntryPoints::getCurrentAccessibleValueFromContext(jobject AccessibleContext, wchar_t *value, short len) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getCurrentAccessibleValueFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getMaximumAccessibleValueFromContext(jobject AccessibleContext, wchar_t *value, short len) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getMaximumAccessibleValueFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getMinimumAccessibleValueFromContext(jobject AccessibleContext, wchar_t *value, short len) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getMinimumAccessibleValueFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::addAccessibleSelectionFromContext(jobject AccessibleContext, int i) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::addAccessibleSelectionFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::clearAccessibleSelectionFromContext(jobject AccessibleContext) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::clearAccessibleSelectionFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleSelectionFromContext(jobject AccessibleContext, int i) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleSelectionFromContext(%x):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::getAccessibleSelectionCountFromContext(jobject AccessibleContext) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::getAccessibleSelectionCountFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::isAccessibleChildSelectedFromContext(jobject AccessibleContext, int i) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::isAccessibleChildSelectedFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::removeAccessibleSelectionFromContext(jobject AccessibleContext, int i) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::removeAccessibleSelectionFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::selectAllAccessibleSelectionFromContext(jobject AccessibleContext) {
    ...
    PrintDebugString("\r\nCalling AccessBridgeJavaEntryPoints::selectAllAccessibleSelectionFromContext(%X):", AccessibleContext);
...
AccessBridgeJavaEntryPoints::addJavaEventNotification(jlong type) {
    ...
    PrintDebugString("\r\n  in AccessBridgeJavaEntryPoints::addJavaEventNotification(%X);", type);
...
AccessBridgeJavaEntryPoints::removeJavaEventNotification(jlong type) {
    ...
    PrintDebugString("\r\n  in AccessBridgeJavaEntryPoints::removeJavaEventNotification(%X):", type);
...
AccessBridgeJavaEntryPoints::addAccessibilityEventNotification(jlong type) {
    ...
    PrintDebugString("\r\n  in AccessBridgeJavaEntryPoints::addAccessibilityEventNotification(%X);", type);

    // Let AccessBridge know we want to add an event type
    if (addAccessibilityEventNotificationMethod != (jmethodID) 0) {
        PrintDebugString("\r\n     addAccessibilityEventNotification: calling void method: accessBridgeObject = %X", accessBridgeObject);
        jniEnv->CallVoidMethod(accessBridgeObject,
...
AccessBridgeJavaEntryPoints::removeAccessibilityEventNotification(jlong type) {
    ...
    PrintDebugString("\r\n  in AccessBridgeJavaEntryPoints::removeAccessibilityEventNotification(%X):", type);
