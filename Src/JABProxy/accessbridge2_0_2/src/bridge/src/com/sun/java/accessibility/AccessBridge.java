...
/*
 * @(#)AccessBridge.java	1.65 06/06/22
 * 
...
public class AccessBridge implements Runnable {
    ...
    public static final String AccessBridgeVersion = 
	"AccessBridge 2.0";
    ...
    /**
       ...
       will use the extended search algorithm. // Ben Key
    */
    ...
    /**
       ...
       extendedVirtualNameSearchRoles array. // Ben Key
    */
    ...
    /*
     * Initialize the hashtable that maps Strings to AccessibleRoles. // Ben Key
     */
    private void initAccessibleRoleMap() {
	/*
	  initialize AccessibleRoles map

	  Note to Lynn Monsanto from Ben Key.  I am now using the methods in java.lang.reflect.*
	  to build this map and I have completely removed the allAccessibleRoles array.  I made this
	  change for several reasons:
	  (1)  This change allows the accessibleRoleMap to be built dynamically, based on what
	  accessible roles are available to the version of the Java VM that is currently in use.
	  (2)  It is the only way to add Accessible Roles that are only available in J2SE version
	  1.5 and above to the accessibleRoleMap.
	  (3)  This change was required to provide compatibility with J2SE version 1.2.x.
	  Without this change you get the following exception whenever you attempt to run a Swing
	  application or applet:
	  Exception in thread "main" java.lang.NoSuchFieldError: javax.accessibility.AccessibleRole:
          field CANVAS not found. This problem has existed since Access Bridge version 1.1.
	*/
	try
	    {
	...
	*/
     ...
     */
    public native void sendDebugString(String debugStr);
    ...
    public native int jawtGetNativeWindowHandleFromComponent(Component comp);

    public native Component jawtGetComponentFromNativeWindowHandle(int handle);
	...
	if (useJAWT_DLL) {
	    ...
	    try {
		javaGetComponentFromNativeWindowHandleMethod = toolkit.getClass().getMethod(
	    ...
	    // is present if JAWT.DLL is not installed
	    try {
		javaGetNativeWindowHandleFromComponentMethod = toolkit.getClass().getMethod(
    ...
    static private Hashtable windowHandleToContextMap = new Hashtable();

    // hash table of AccessibleContext to native window handle mappings
    static private Hashtable contextToWindowHandleMap = new Hashtable();

    /*
     * adds a virtual window handler to our hash tables
     */
    static public void registerVirtualFrame( Accessible a, 
					     Integer nativeWindowHandle ) {
	if (a != null) {
	    windowHandleToContextMap.put(nativeWindowHandle, 
					 a.getAccessibleContext());

	    contextToWindowHandleMap.put(a.getAccessibleContext(), 
					 nativeWindowHandle);
	}
    }
     ...
     */
    static public void revokeVirtualFrame( Accessible a, 
					   Integer nativeWindowHandle ) {
	...
	theAccessBridge.debugString("addNativeWindowHandler="+handler.getClass());
	if (handler == null) {
	...
	theAccessBridge.debugString("removeNativeWindowHandler="+handler.getClass());
	if (handler == null) {
	    throw new IllegalArgumentException();
	}
	...
	if (o instanceof AccessibleContext) {
	    saveContextToWindowHandleMapping((AccessibleContext)o, nativeHandle);
	...
	for (int i = 0; i < numHandlers; i++) {
	    NativeWindowHandler nextHandler = 
		(NativeWindowHandler)nativeWindowHandlers.elementAt(i);
	    Accessible a = 
		nextHandler.getAccessibleFromNativeWindowHandle(nativeHandle);
	    if (a != null) {
		saveContextToWindowHandleMapping(a.getAccessibleContext(), nativeHandle);
	...
	while (keys.hasMoreElements()) {
	    debugString(" next elem = "+keys.nextElement());
	}
	Object o = contextToWindowHandleMap.get(ac);
	if (o instanceof Integer) {
	    debugString(" returning (success) "+ ((Integer)o).intValue());
	 ...
	 */
	public Accessible getAccessibleFromNativeWindowHandle(int nativeHandle) {
	    Component c = getComponentFromNativeWindowHandle(nativeHandle);
	    if (c instanceof Accessible) {
		saveContextToWindowHandleMapping(((Accessible)c).getAccessibleContext(), 
						 nativeHandle);
		return (Accessible)c;
	    } else {
		return null;
	 ...
	 * map an HWND to an AWT Component
	 */
	protected Component getComponentFromNativeWindowHandle(int nativeHandle) {
	    if (useJAWT_DLL) {
		...
		if (javaGetComponentFromNativeWindowHandleMethod != null) {
			...
			Object o = 
			    javaGetComponentFromNativeWindowHandleMethod.invoke(toolkit, args);
			if (o instanceof Accessible) {
			    saveContextToWindowHandleMapping(((Accessible)o).getAccessibleContext(), 
							     nativeHandle);
			}
			return (Component)o;
		    } catch (InvocationTargetException e) {
     ...
     */
    public AccessibleContext getAccessibleContextAt(int x, int y, 
     ...
     */
    public AccessibleContext getAccessibleContextAt_1(int x, int y, 
						      AccessibleContext parent) {
	debugString(" : getAccessibleContextAt_1 called");
	debugString("   -> x = " + x + " y = " + y + " parent = " + parent);
	
	if (parent != null) {
	    AccessibleComponent acmp = parent.getAccessibleComponent();
	    if (acmp != null) {
		Point loc = acmp.getLocation();
		Accessible a = acmp.getAccessibleAt(new Point(x - loc.x, y - loc.y));
		if (a != null) {
		    AccessibleContext foundAC = a.getAccessibleContext();
		    if (foundAC != null) {
			if (foundAC != parent) {
			    ...
			    debugString("   returning ac = " + foundAC);
			    return foundAC;
			}
		    }
		}
     ...
     * AWT/Swing version
     */
    public AccessibleContext getAccessibleContextAt_2(int x, int y, 
						      AccessibleContext parent) {
    ...
    public String getVirtualAccessibleNameFromContext (AccessibleContext ac)
    {
				...
				if ( (null != ai) && (ai.length > 0) )
				    ...
				    if ( null != parentContext )
				        ...
				        if ( AccessibleRole.TABLE == parentRole )
				            ...
				            acTableCell = getAccessibleChildFromContext (parentContext, indexInParent);
				 ...
				 (AccessibleRole.TOGGLE_BUTTON == role)
		...
		*/
		...
		if ( (null != version) && (version.compareTo ("1.3") >= 0) )
			...
			if ( (ars.size () > 0) && (ars.contains (AccessibleRelation.LABELED_BY)) )
			    {
		...
		while (childIndex >= 0)
			...
			if ( null != child )
				...
				if ( null != childContext )
				    {
					childRole = childContext.getAccessibleRole ();
			...
			while (childIndex >= 0)
				...
				if ( null != child )
					...
					if ( null != childContext )
					    {
						childRole = childContext.getAccessibleRole ();
						if ( ( AccessibleRole.PUSH_BUTTON == childRole ) || ( AccessibleRole.TOGGLE_BUTTON == childRole ) )
						    {
							childX = getAccessibleXcoordFromContext (childContext);
							childY = getAccessibleYcoordFromContext (childContext);
							childWidth = getAccessibleWidthFromContext (childContext);
	...
	if (ac != null) {
		...
		if (s != null) {
		    references.increment(s);
                    debugString("Returning AccessibleRole in en_US from Context: " + s);
		    return s;
                }
            }
	} else {
	...
	if (ac != null) {
		...
		if (s != null && 
		    s.indexOf(_AccessibleState.MANAGES_DESCENDANTS.toDisplayString(Locale.US)) == -1) {
			...
			s += ",";
			s += _AccessibleState.MANAGES_DESCENDANTS.toDisplayString(Locale.US);
		    } 
		    references.increment(s);
	...
	if (ac != null) {
            debugString("Returning Accessible IndexInParent from Context: " + ac.getAccessibleIndexInParent());
     ...
     */
    public int getAccessibleChildrenCountFromContext(AccessibleContext ac) {
	if (ac != null) {
            debugString("Returning Accessible ChildrenCount from Context: " + ac.getAccessibleChildrenCount());
     ...
     */
    public AccessibleContext getAccessibleChildFromContext(AccessibleContext ac, int index) {
	...
	if (parent instanceof Accessible) {
		...
		int row = getAccessibleTableRow(at, index);
		int column = getAccessibleTableColumn(at, index);
            ...
            if (acmp != null) {
                ...
                if (r != null) {
                    try {
                        ...
                        debugString("*** Caught EXCEPTION in getAccessibleBoundsOnScreenFromContext: " + e.toString());
                    }
                }
            }
	} else {
	...
	if (ac != null) {
            ...
            if (as != null) {
                debugString("returning AccessibleSelection Context="+as.getClass());
                return as;
            }
	} else {
	...
	if (ac != null) {
	    ...
	    Accessible parent = ac.getAccessibleParent();
	    debugString("  parent = "+parent.getClass());
	    if (parent instanceof Accessible) {
		int indexInParent = ac.getAccessibleIndexInParent();
		debugString("  indexInParent = "+indexInParent);
		Accessible child = 
		    parent.getAccessibleContext().getAccessibleChild(indexInParent);
     ...
     */
    public int getAccessibleIndexAtPointFromContext(AccessibleContext ac, 
						    int x, int y) {
	...
	if (ac != null) {
		...
		try {
		    ...
		    debugString("  getLocationOnScreen = "+p);
		    if (p != null) {
			x = x - p.x;
			if (x < 0) {
			    x = 0;
			...
			debugString("  newPoint = "+newPoint);
			
			int indexAtPoint = at.getIndexAtPoint(new Point(x, y));
			debugString("  indexAtPoint = "+indexAtPoint);
			return indexAtPoint;
		    }
		} catch (Exception e) {
		}
	...
	if (ac != null) {
            AccessibleText at = ac.getAccessibleText();
            if (at != null) {
                String s = at.getAtIndex(AccessibleText.WORD, index);
                if (s != null) {
                    references.increment(s);
                    return s;
                }
	...
	if (ac != null) {
            AccessibleText at = ac.getAccessibleText();
            if (at != null) {
                String s = at.getAtIndex(AccessibleText.SENTENCE, index);
                if (s != null) {
                    references.increment(s);
                    return s;
                }
    ...
    /**
     * return the selcted text from an AccessibleContext
     */
    public String getAccessibleTextSelectedTextFromContext(AccessibleContext ac) {
	if (ac != null) {
            AccessibleText at = ac.getAccessibleText();
            if (at != null) {
                String s = at.getSelectedText();
                if (s != null) {
                    references.increment(s);
                    return s;
                }
     ...
     *
     * algorythm:  cast back, doubling each time,
	...
	if (ac != null) {
            AccessibleValue av = ac.getAccessibleValue();
            if (av != null) {
                Number value = av.getMaximumAccessibleValue();
                if (value != null) {
                    String s = value.toString();
                    if (s != null) {
                        references.increment(s);
	...
	if (ac != null) {
            AccessibleValue av = ac.getAccessibleValue();
            if (av != null) {
                Number value = av.getMinimumAccessibleValue();
                if (value != null) {
                    String s = value.toString();
                    if (s != null) {
                        references.increment(s);
     ...
     *
     */
    void addAccessibleSelectionFromContext(AccessibleContext ac, int i) {
	if (ac != null) {
            AccessibleSelection as = ac.getAccessibleSelection();
            if (as != null) {
     ...
     */
    public AccessibleContext getAccessibleTableCellAccessibleContext(AccessibleTable at,
								     int row, int column) {
	...
	if (at == null) {
	...
	} else if (!(at instanceof AccessibleContext)) {
		...
		if (child instanceof JTable) {
			...
			renderer.getTableCellRendererComponent(table, table.getValueAt(row, column),
							       false, false, row, column);
		    if (component instanceof Accessible) {
			debugString("getAccessibleTableCellAccessibleContext returning = "+component.getClass());
			return ((Accessible)component).getAccessibleContext();
		    }
		}
	    }
     ...
     */
    public boolean isAccessibleTableCellSelected(AccessibleTable at, int row, 
						 int column) {
	...
	if (at != null) {
	    ...
	    debugString(" ##### isAccessibleTableCellSelected="+isSelected);
	    return isSelected;
	}
	...
	if (ac != null) {
	    AccessibleTable atRowHeader = getAccessibleTableRowHeader(ac);
	    if (atRowHeader != null) {
	...
	if (ac != null) {
	    AccessibleTable atColumnHeader = getAccessibleTableColumnHeader(ac);
     ...
     */
    public AccessibleContext getAccessibleTableRowDescription(AccessibleTable table,
							      int row) {
	if (table != null) {
     ...
     */
    public AccessibleContext getAccessibleTableColumnDescription(AccessibleTable at,
								 int column) {
	if (at != null) {
     ...
     */
    public boolean isAccessibleTableRowSelected(AccessibleTable at, 
						int row) {
	if (at != null) {
     ...
     */
    public boolean isAccessibleTableColumnSelected(AccessibleTable at, 
						   int column) {
	if (at != null) {
	...
	if (at instanceof AccessibleExtendedTable) {
	    // workaround for JTable bug in JDK 1.4.0
	    // return ((AccessibleExtendedTable)at).getAccessibleRow(index);
	    return index / at.getAccessibleColumnCount();
	}
	...
	if (at instanceof AccessibleExtendedTable) {
	    // workaround for JTable bug in JDK 1.4.0
	    // return ((AccessibleExtendedTable)at).getAccessibleColumn(index);
	    return index % at.getAccessibleColumnCount();
	}
	...
	if (at instanceof AccessibleExtendedTable) {
	    // workaround for JTable bug in JDK 1.4.0
	    // return ((AccessibleExtendedTable)at).getAccessibleIndex(row, column);
	    return row * at.getAccessibleColumnCount() + column;
	}
     ...
     * AccessibleRelationSet
     */
    public AccessibleContext getAccessibleRelationTarget(AccessibleContext ac, 
							 int i, int j) {
	...
	if (ac != null) {
	    ...
	    if (i >= 0 && i < relations.length) {
		Object [] targets = relations[i].getTarget();
		if (j >= 0 & j < targets.length) {
		    Object o = targets[j];
		    debugString("***** target="+o.getClass());
		    if (o instanceof Accessible) {
	...
	AccessibleHypertext hypertext = getAccessibleHypertext(ac);
	if (hypertext == null) {
	    return 0;
	}
     ...
     */
    AccessibleHyperlink getAccessibleHyperlink(AccessibleHypertext hypertext, 
					       int i) {
	debugString("getAccessibleHyperlink");
	if (hypertext == null) {
	    return null;
     ...
     */
    boolean activateAccessibleHyperlink(AccessibleContext ac,
	...
	if (parent instanceof Accessible) {
		...
		debugString("getAccelerator = "+keyStroke);
		return keyStroke;
	    }
	}
	return null;
	...
	debugString("getModifiers: text = "+text+"; modifiers = "+modifiers);
        return modifiers;
    }
	...
	debugString("getAccessibleKeyBindingChar: index = "+index);
	...
	debugString("getAccessibleKeyBindingModifiers: index = "+index);
	if (ac == null  || (! runningOnJDK1_4) ) {
	    return 0;
	}
	if (index == 0) {	// mnemonic
	...
	debugString("   icon description    = "+
		    ai[index].getAccessibleIconDescription());
	...
	debugString("   icon height    = "+
		    ai[index].getAccessibleIconHeight());
	...
	debugString("   icon height    = "+
		    ai[index].getAccessibleIconWidth());
	...
	debugString("   #actions = "+aa.getAccessibleActionCount());
	...
	debugString("   action name    = "+
		    aa.getAccessibleActionDescription(index));
	...
	if (index == -1) {
	    debugString("  no such action: "+name);
	    return false;
	}
	debugString(" doing action #"+index+": "+name);
	boolean retval = aa.doAccessibleAction(index);
	debugString(" action "+name+": "+retval);
	return retval;
    }
	...
	Accessible parent = ac.getAccessibleParent();
	if (parent == null) {
	    debugString("   getTopLevelObject; returning: "+ac);
	    return ac;
	}
	...
	while (tmp != null && tmp.getAccessibleContext() != null) {
	    ...
	    debugString("   getTopLevelObject; next parent = "+
			tmp.getAccessibleContext().getClass());
	    parent = tmp;
	    tmp = parent.getAccessibleContext().getAccessibleParent();
	}
	debugString("   getTopLevelObject; returning: "+
		    parent.getAccessibleContext().getClass());
	return parent.getAccessibleContext();
    }
     ...
     */
    public AccessibleContext getParentWithRole (AccessibleContext ac, 
						String roleName) {
	debugString("getParentWithRole; ac = "+ac+"; role = "+roleName);
	if (ac == null || roleName == null) {
	    return null;
	}

	AccessibleRole role = (AccessibleRole)accessibleRoleMap.get(roleName);
	if (role == null) {
	    debugString("   getParentWithRole; no such role: "+roleName);
	    return ac;
	}

	Accessible parent = ac.getAccessibleParent();
	if (parent == null && ac.getAccessibleRole() == role) {
	    debugString("   getParentWithRole; returning ac: "+ac.getClass());
	    return ac;
	}
	...
	while (tmp != null && (tmp_ac = tmp.getAccessibleContext()) != null) {
	    debugString("   getParentWithRole; next parent = "+tmp_ac.getClass());
	    AccessibleRole ar = tmp_ac.getAccessibleRole();
	    if (ar == role) {
		// found
		debugString("   getParentWithRole; returning tmp_ac: "+tmp_ac.getClass());
		return tmp_ac;
	    }
	    parent = tmp;
	    tmp = parent.getAccessibleContext().getAccessibleParent();
     ...
     */
    AccessibleContext getParentWithRoleElseRoot (AccessibleContext ac,
						 String roleName) {
	...
	Accessible parent = ac.getAccessibleParent();
	if (parent == null) {
	    debugString("   getObjectDepth: count = "+count);
	    return count;
	}
	Accessible tmp = parent;
	while (tmp != null && tmp.getAccessibleContext() != null) {
	...
	debugString("   getActiveDescendent: returning: "+a.getAccessibleContext());
	return a.getAccessibleContext();
    }
	...
	debugString("    getJAWSAccessibleName: name = "+name);
	return name;
    }
	...
	acomp.requestFocus();
	boolean result = 
	    ac.getAccessibleStateSet().contains(AccessibleState.FOCUSED);
	debugString("    GGGG requestFocus: result = "+result);
	return result;
    }
	...
	debugString("    selectTextRange: result = "+result);
	return result;
    }
	...
	debugString("    setCaretPosition: result = "+result);
	return result;
    }
	...
	for (int i = 0; i < numChildren; i++) {
	    ...
	    if (ac2 == null ||
		(!ac2.getAccessibleStateSet().contains(AccessibleState.SHOWING))) {
		continue;
	    }
	...
	if (_visibleChild != null) {
	    debugString("    getVisibleChild: found child = "+
			_visibleChild.getAccessibleName());
	}
	...
	public void decrement(Object o) {
	    Reference aRef = (Reference) refs.get(o);
	    if (aRef != null) {
		aRef.value--;
		if (aRef.value == 0) {
		    refs.remove(o);
   ...
   /**
     ...
     */
    public native void propertyCaretChange(PropertyChangeEvent e, 
					   AccessibleContext src,
					   int oldValue, int newValue);
    public native void propertyDescriptionChange(PropertyChangeEvent e, 
						 AccessibleContext src,
						 String oldValue, String newValue);
    public native void propertyNameChange(PropertyChangeEvent e, 
					  AccessibleContext src,
					  String oldValue, String newValue);
    public native void propertySelectionChange(PropertyChangeEvent e, 
					       AccessibleContext src);
    public native void propertyStateChange(PropertyChangeEvent e, 
					   AccessibleContext src,
					   String oldValue, String newValue);
    public native void propertyTextChange(PropertyChangeEvent e, 
					  AccessibleContext src);
    public native void propertyValueChange(PropertyChangeEvent e, 
					   AccessibleContext src,
					   String oldValue, String newValue);
    public native void propertyVisibleDataChange(PropertyChangeEvent e, 
						 AccessibleContext src);
    public native void propertyChildChange(PropertyChangeEvent e, 
					   AccessibleContext src,
					   AccessibleContext oldValue,
					   AccessibleContext newValue);
    public native void propertyActiveDescendentChange(PropertyChangeEvent e, 
						      AccessibleContext src,
						      AccessibleContext oldValue,
						      AccessibleContext newValue);
     ...
     */
    public native void focusGained(FocusEvent e, AccessibleContext src);
    public native void focusLost(FocusEvent e, AccessibleContext src);

    /**
     * native method for handling caret events
     */
    public native void caretUpdate(CaretEvent e, AccessibleContext src);

    /**
     * native methods for handling mouse events
     */
    public native void mouseClicked(MouseEvent e, AccessibleContext src);
    public native void mouseEntered(MouseEvent e, AccessibleContext src);
    public native void mouseExited(MouseEvent e, AccessibleContext src);
    public native void mousePressed(MouseEvent e, AccessibleContext src);
    public native void mouseReleased(MouseEvent e, AccessibleContext src);

    /**
     * native methods for handling menu & popupMenu events
     */
    public native void menuCanceled(MenuEvent e, AccessibleContext src);
    public native void menuDeselected(MenuEvent e, AccessibleContext src);
    public native void menuSelected(MenuEvent e, AccessibleContext src);
    public native void popupMenuCanceled(PopupMenuEvent e, AccessibleContext src);
    public native void popupMenuWillBecomeInvisible(PopupMenuEvent e, 
						    AccessibleContext src);
    public native void popupMenuWillBecomeVisible(PopupMenuEvent e, 
						  AccessibleContext src);
    ...
    public static final long FOCUS_EVENTS = (FOCUS_GAINED_EVENTS | FOCUS_LOST_EVENTS);

    public static final long CARET_UPATE_EVENTS = 8;
    public static final long CARET_EVENTS = CARET_UPATE_EVENTS;
    ...
    public static final long MOUSE_EVENTS = (MOUSE_CLICKED_EVENTS | MOUSE_ENTERED_EVENTS |
    ...
    public static final long MENU_EVENTS = (MENU_CANCELED_EVENTS | MENU_DESELECTED_EVENTS |
    ...
    public static final long POPUPMENU_EVENTS = (POPUPMENU_CANCELED_EVENTS |
    ...
    public static final long PROPERTY_EVENTS = (PROPERTY_NAME_CHANGE_EVENTS |
    ...
    class EventHandler implements PropertyChangeListener,
        ...
        public EventHandler(AccessBridge bridge) {
            accessBridge = bridge;
	    ...
	    if ((accessibilityEventMask & PROPERTY_EVENTS) != 0) {
                ...
                if (ac != null) {
                    ...
                    if (propertyName.compareTo(AccessibleContext.ACCESSIBLE_CARET_PROPERTY) == 0) {
                        ...
                        accessBridge.debugString(" - about to call propertySelectionChange()");
	 ...
	 */
	AccessibleContext prevAC = null; // previous AccessibleContext
	...
	private void handleActiveDescendentEvent(PropertyChangeEvent e,
		...
		a = Translator.getAccessible((Component) e.getOldValue());
		if (a != null) {
		...
		a = Translator.getAccessible((Component) e.getNewValue());
		if (a != null) {
	...
	private boolean stateChangeListenerAdded = false;
		...
		if ((javaEventMask & FOCUS_GAINED_EVENTS) != 0) {
		    Accessible a = Translator.getAccessible(e.getSource());
		    if (a != null) {
			accessBridge.focusGained(e, a.getAccessibleContext());
		    }
		}
	    }
	}
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (path.length > 1) {
			...
			FocusEvent e = 
			    new FocusEvent(penult, FocusEvent.FOCUS_GAINED);
			accessBridge.focusGained(e, ((Accessible)penult).getAccessibleContext());
		    } else if (penult instanceof JPopupMenu) {
			// This is a popup with an item selected
			FocusEvent e = 
			    new FocusEvent(last, FocusEvent.FOCUS_GAINED);
			accessBridge.focusGained(e, ((Accessible)last).getAccessibleContext());
		    }
		}
	    } else {
		...
		if (focusOwner instanceof Accessible) {
		    ...
		    accessBridge.focusGained(e, ((Accessible)focusOwner).getAccessibleContext());
		}
	    }
        }

        public void focusLost(FocusEvent e) {
	    if ((javaEventMask & FOCUS_LOST_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.focusLost(e, a.getAccessibleContext());
                }
            }
        }
	 ...
	 */
	    ...
	    if ((javaEventMask & CARET_UPATE_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.caretUpdate(e, a.getAccessibleContext());
                }
            }
        }
	 ...
	 */
	    ...
	    if ((javaEventMask & MOUSE_CLICKED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.mouseClicked(e, a.getAccessibleContext());
                }
            }
        }

        public void mouseEntered(MouseEvent e) {
	    if ((javaEventMask & MOUSE_ENTERED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.mouseEntered(e, a.getAccessibleContext());
                }
            }
        }

        public void mouseExited(MouseEvent e) {
	    if ((javaEventMask & MOUSE_EXITED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.mouseExited(e, a.getAccessibleContext());
                }
            }
        }

        public void mousePressed(MouseEvent e) {
	    if ((javaEventMask & MOUSE_PRESSED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.mousePressed(e, a.getAccessibleContext());
                }
            }
        }

        public void mouseReleased(MouseEvent e) {
	    if ((javaEventMask & MOUSE_RELEASED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.mouseReleased(e, a.getAccessibleContext());
                }
            }
        }
	 ...
	 */
	    ...
	    if ((javaEventMask & MENU_CANCELED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.menuCanceled(e, a.getAccessibleContext());
                }
            }
        }

        public void menuDeselected(MenuEvent e) {
	    if ((javaEventMask & MENU_DESELECTED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.menuDeselected(e, a.getAccessibleContext());
                }
            }
        }

        public void menuSelected(MenuEvent e) {
	    if ((javaEventMask & MENU_SELECTED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.menuSelected(e, a.getAccessibleContext());
                }
            }
        }

        public void popupMenuCanceled(PopupMenuEvent e) {
	    if ((javaEventMask & POPUPMENU_CANCELED_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.popupMenuCanceled(e, a.getAccessibleContext());
                }
            }
        }

        public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
	    if ((javaEventMask & POPUPMENU_WILL_BECOME_INVISIBLE_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.popupMenuWillBecomeInvisible(e, a.getAccessibleContext());
                }
            }
        }

        public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
	    if ((javaEventMask & POPUPMENU_WILL_BECOME_VISIBLE_EVENTS) != 0) {
                Accessible a = Translator.getAccessible(e.getSource());
                if (a != null) {
                    accessBridge.popupMenuWillBecomeVisible(e, a.getAccessibleContext());
                }
            }
        }
    }
	...
	*/
	...
	private TreePath getChildTreePath(int i) {
	    ...
	    if (i < 0 || i >= getAccessibleChildrenCount() || path == null) {
		return null;
	    } else {
...
class _AccessibleState extends AccessibleState {
	...
	public static final _AccessibleState MANAGES_DESCENDANTS
            = new _AccessibleState ("managesDescendents"); 
