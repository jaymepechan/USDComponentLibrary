...
/*
 * @(#)WinAccessBridge.cpp	1.32 05/08/22
 */
    ...
    BOOL CALLBACK AccessBridgeDialogProc(HWND hDlg, UINT message, UINT wParam, LONG lParam) {
        ...
        case WM_COPYDATA:
            if (theDialogWindow == (HWND) wParam) {
                PrintDebugString("\r\nAccessBridgeDialogProc -> Got WM_COPYDATA from Java Bridge DLL");
            } else {
                PrintDebugString("\r\nAccessBridgeDialogProc -> Got WM_COPYDATA from HWND %x", wParam); 
        ...
        case AB_MESSAGE_QUEUED:
            PrintDebugString("\r\nAccessBridgeDialogProc -> Got AB_MESSAGE_QUEUED from ourselves");
            theWindowsAccessBridge->receiveAQueuedPackage();
            break;

            // a JavaAccessBridge DLL is going away
        case AB_DLL_GOING_AWAY:
            PrintDebugString("\r\n***** AccessBridgeDialogProc -> Got AB_DLL_GOING_AWAY message");
        ...
        default:
            ...
            if (message == theFromJavaHelloMsgID) {
                PrintDebugString("\r\nAccessBridgeDialogProc -> Got theFromJavaHelloMsgID; wParam = %X, lParam = %X", wParam, lParam);
...
WinAccessBridge::~WinAccessBridge() {
    ...
    PrintDebugString("\r\n*****in WinAccessBridge::~WinAccessBridge()"); 
    ...
    while (current != (AccessBridgeJavaVMInstance *) 0) {
        PrintDebugString("  telling %X we're going away", current->javaAccessBridgeWindow); 
...
WinAccessBridge::initWindow() {
    ...
    // DEBUG(ShowWindow (theDialogWindow, SW_SHOWNORMAL));
    // DEBUG(UpdateWindow (theDialogWindow)); 
...
WinAccessBridge::rendezvousWithNewJavaDLL(HWND JavaBridgeDLLwindow, long vmID) {
    ...
    PrintDebugString("\r\nin JavaAccessBridge::rendezvousWithNewJavaDLL(%X, %X)",
...
WinAccessBridge::queuePackage(char *buffer, long bufsize) {
    PrintDebugString("  in WinAccessBridge::queuePackage(%X, %d)", buffer, bufsize); 
...
WinAccessBridge::receiveAQueuedPackage() {
    ...
    PrintDebugString("\r\nin WinAccessBridge::receiveAQueuedPackage()");
    ...
    if (messageQueue->getRemoveLockSetting() == FALSE) {
        ...
        DEBUG(PackageType *type = (PackageType *) element->buffer); 
        DEBUG(FocusGainedPackageTag *pkg = (FocusGainedPackageTag *) (((char *) element->buffer) + sizeof(PackageType))); 
        DEBUG(PrintDebugString("     PackageType = %X", *type)); 
        DEBUG(PrintDebugString("     EventPackage: vmID = %X, event = %X, source = %X", pkg->vmID, pkg->Event, pkg->AccessibleContextSource)); 
...
WinAccessBridge::preProcessPackage(char *buffer, long bufsize) {
    PrintDebugString("\r\nPreProcessing package sent from Java:");
...
#define DISPATCH_EVENT_PACKAGE(packageID, eventPackage, fireEventMethod)			\
	case packageID:																	\
		PrintDebugString("   type == <some type>");				                \
		if (bufsize == sizeof(PackageType) + sizeof(eventPackage)) {				\
			...
			PrintDebugString("   call successful");			                    \
...
WinAccessBridge::processPackage(char *buffer, long bufsize) {
    PrintDebugString("\r\nWinAccessBridge::Processing package sent from Java:");
...
WinAccessBridge::JavaVMDestroyed(HWND VMBridgeDLLWindow) {
    PrintDebugString("\r\n***** WinAccessBridge::JavaVMDestroyed(%X)", VMBridgeDLLWindow);
 ...
 */
void
WinAccessBridge::releaseJavaObject(long vmID, jobject object) {
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
...
WinAccessBridge::getVersionInfo(long vmID, AccessBridgeVersionInfo *info) {
    ...
    *type = cGetAccessBridgeVersionPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getVersionInfo(%X, )", vmID);
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
...
WinAccessBridge::isJavaWindow(HWND window) {
    ...
    *type = cIsJavaWindowPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::isJavaWindow(%X)", window);
 ...
 */
BOOL
WinAccessBridge::isSameObject(long vmID, jobject obj1, jobject obj2) {

    PrintDebugString("\r\nWinAccessBridge::isSameObject(%X %X)", obj1, obj2);
    ...
    *type = cIsSameObjectPackage;
    ...
    if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
        if (pkg->rResult != 0) {
            ...
            PrintDebugString("  WinAccessBridge::isSameObject returning FALSE (di");
            return FALSE;
        }
    }
 ...
 */
BOOL
WinAccessBridge::getAccessibleContextFromHWND(HWND window, long *vmID, jobject *AccessibleContext) {
    PrintDebugString("  in WinAccessBridge::getAccessibleContextFromHWND");
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return FALSE;
    }
    ...
    *type = cGetAccessibleContextFromHWNDPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleContextFromHWND(%X, %X, )", window, vmID);

    DEBUG(pkg->rVMID = (long ) 0x01010101);
    DEBUG(pkg->rAccessibleContext = (jobject) 0x01010101);
            ...
            if (pkg->rAccessibleContext != 0) {
                ...
                *AccessibleContext = pkg->rAccessibleContext;
                if (pkg->rVMID != current->vmID) {
                    ...
                    PrintDebugString("   -> pkg->rAccessibleContext = %X", pkg->rAccessibleContext);
                    return FALSE;
                }
                return TRUE;
            }
 ...
 */
HWND
WinAccessBridge::getHWNDFromAccessibleContext(long vmID, jobject accessibleContext) {
    ...
    *type = cGetHWNDFromAccessibleContextPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getHWNDFromAccessibleContext(%X)", accessibleContext);
 ...
 */
BOOL
WinAccessBridge::getAccessibleContextAt(long vmID, jobject AccessibleContextParent, 
    ...
    *type = cGetAccessibleContextAtPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleContextAt(%X, %X, %d, %c)", vmID, AccessibleContextParent, x, y);
    HWND destABWindow = javaVMs->findAccessBridgeWindow(pkg->vmID);
    if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
        *AccessibleContext = pkg->rAccessibleContext;
        return TRUE;
 ...
 */
BOOL
WinAccessBridge::getAccessibleContextWithFocus(HWND window, long *vmID, jobject *AccessibleContext) {
    ...
    *type = cGetAccessibleContextWithFocusPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleContextWithFocus(%X, %X, )", window, vmID);
    // find vmID, etc. from HWND; ask that VM for the AC w/Focus
    if (getAccessibleContextFromHWND(window, &(pkg->rVMID), &(pkg->rAccessibleContext)) == TRUE) {
        HWND destABWindow = javaVMs->findAccessBridgeWindow(pkg->rVMID);	// ineffecient [[[FIXME]]]
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
            *vmID = pkg->rVMID;
            *AccessibleContext = pkg->rAccessibleContext;
            return TRUE;
...
WinAccessBridge::getAccessibleContextInfo(long vmID, 
    ...
    *type = cGetAccessibleContextInfoPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleContextInfo(%X, %X, )", vmID, AccessibleContext);
...
WinAccessBridge::getAccessibleChildFromContext(long vmID, 
    ...
    *type = cGetAccessibleChildFromContextPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleChildFromContext(%X, %X, %d)", vmID, AccessibleContext, childIndex);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleParentFromContext(long vmID, 
    ...
    *type = cGetAccessibleParentFromContextPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleParentFromContext(%X, %X)", vmID, AccessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTableInfo(long vmID,
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableInfo(%X, %X, %X)", vmID, accessibleContext,
                     tableInfo);
...
BOOL 
WinAccessBridge::getAccessibleTableCellInfo(long vmID, jobject accessibleTable, 
                                            jint row, jint column, 
                                            AccessibleTableCellInfo *tableCellInfo) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableCellInfo(%X, %X, %d, %d, %X)", vmID, 
    ...
    *type = cGetAccessibleTableCellInfoPackage;
    ...
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
            PrintDebugString("  XXXX pkg->rTableCellInfo.accessibleContext = %X", pkg->rTableCellInfo.accessibleContext);
...
BOOL
WinAccessBridge::getAccessibleTableRowHeader(long vmID, jobject accessibleContext, AccessibleTableInfo *tableInfo) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableRowHeader(%X, %X, %X)", vmID, accessibleContext);
...
BOOL
WinAccessBridge::getAccessibleTableColumnHeader(long vmID, jobject accessibleContext, AccessibleTableInfo *tableInfo) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableColumnHeader(%X, %X, %X)", vmID, accessibleContext);
...
WinAccessBridge::getAccessibleTableRowDescription(long vmID,
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableRowDescription(%X, %X, %d)", vmID, accessibleContext,
                     row);
...
WinAccessBridge::getAccessibleTableColumnDescription(long vmID,
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableColumnDescription(%X, %X, %d)", vmID, accessibleContext,
                     column);
...
jint 
WinAccessBridge::getAccessibleTableRowSelectionCount(long vmID, jobject accessibleTable) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableRowSelectionCount(%X, %X, %X)", vmID, accessibleTable);
...
BOOL 
WinAccessBridge::isAccessibleTableRowSelected(long vmID, jobject accessibleTable, jint row) {

    PrintDebugString("\r\n##### WinAccessBridge::isAccessibleTableRowSelected(%X, %X, %X)", vmID, accessibleTable);
...
BOOL 
WinAccessBridge::getAccessibleTableRowSelections(long vmID, jobject accessibleTable, jint count, jint *selections) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableRowSelections(%X, %X, %X)", vmID, accessibleTable);
...
jint 
WinAccessBridge::getAccessibleTableColumnSelectionCount(long vmID, jobject accessibleTable) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableColumnSelectionCount(%X, %X, %X)", vmID, 
                     accessibleTable);
...
BOOL 
WinAccessBridge::isAccessibleTableColumnSelected(long vmID, jobject accessibleTable, jint column) {
    PrintDebugString("\r\n##### WinAccessBridge::isAccessibleTableColumnSelected(%X, %X, %X)", vmID, accessibleTable);
...
BOOL 
WinAccessBridge::getAccessibleTableColumnSelections(long vmID, jobject accessibleTable, jint count, 
                                                    jint *selections) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableColumnSelections(%X, %X, %X)", vmID, accessibleTable);
...
jint 
WinAccessBridge::getAccessibleTableRow(long vmID, jobject accessibleTable, jint index) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableRow(%X, %X, index=%d)", vmID, 
...
jint 
WinAccessBridge::getAccessibleTableColumn(long vmID, jobject accessibleTable, jint index) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableColumn(%X, %X, index=%d)", vmID, 
                     accessibleTable, index);
...
jint 
WinAccessBridge::getAccessibleTableIndex(long vmID, jobject accessibleTable, jint row, jint column) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleTableIndex(%X, %X, row=%d, col=%d)", vmID, 
...
BOOL 
WinAccessBridge::getAccessibleRelationSet(long vmID, jobject accessibleContext, 
                                          AccessibleRelationSetInfo *relationSetInfo) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleRelationSet(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::getAccessibleHypertext(long vmID, jobject accessibleContext, 
                                        AccessibleHypertextInfo *hypertextInfo) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleHypertext(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::activateAccessibleHyperlink(long vmID, jobject accessibleContext, 
                                             jobject accessibleHyperlink) {

    PrintDebugString("\r\nWinAccessBridge::activateAccessibleHyperlink(%X %X)", accessibleContext,
                     accessibleHyperlink);
...
WinAccessBridge::getAccessibleHyperlinkCount(const long vmID, 
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleHyperlinkCount(%X, %X)", 
...
WinAccessBridge::getAccessibleHypertextExt(const long vmID,
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleHypertextExt(%X, %X, %X)", vmID, 
                     accessibleContext, hypertextInfo);
...
WinAccessBridge::getAccessibleHypertextLinkIndex(const long vmID, 
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleHypertextLinkIndex(%X, %X)", 
                     vmID, hypertext);
...
WinAccessBridge::getAccessibleHyperlink(const long vmID, 
    ...
    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleHyperlink(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::getAccessibleKeyBindings(long vmID, jobject accessibleContext, 
                                          AccessibleKeyBindings *keyBindings) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleKeyBindings(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::getAccessibleIcons(long vmID, jobject accessibleContext, AccessibleIcons *icons) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleIcons(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::getAccessibleActions(long vmID, jobject accessibleContext, AccessibleActions *actions) {

    PrintDebugString("\r\n##### WinAccessBridge::getAccessibleActions(%X, %X, %X)", vmID, 
...
BOOL 
WinAccessBridge::doAccessibleActions(long vmID, jobject accessibleContext, 
                                     AccessibleActionsToDo *actionsToDo, jint *failure) {

    PrintDebugString("\r\nWinAccessBridge::doAccessibleActions(%X #actions %d %s)", accessibleContext,
...
WinAccessBridge::setTextContents (const long vmID, const AccessibleContext accessibleContext,
    ...
    *type = cSetTextContentsPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::setTextContents(%X, %X %ls)", vmID, accessibleContext, text);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getParentWithRole (const long vmID, const AccessibleContext accessibleContext, const wchar_t *role) {
    ...
    *type = cGetParentWithRolePackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getParentWithRole(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getTopLevelObject (const long vmID, const AccessibleContext accessibleContext) {
    ...
    *type = cGetTopLevelObjectPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getTopLevelObject(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getParentWithRoleElseRoot (const long vmID, const AccessibleContext accessibleContext, const wchar_t *role) {
    ...
    *type = cGetParentWithRoleElseRootPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getParentWithRoleElseRoot(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getObjectDepth (const long vmID, const AccessibleContext accessibleContext) {
    ...
    *type = cGetObjectDepthPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getObjectDepth(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getActiveDescendent (const long vmID, const AccessibleContext accessibleContext) {
    ...
    *type = cGetActiveDescendentPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getActiveDescendent(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getVirtualAccessibleName(long vmID, AccessibleContext accessibleContext, 
    ...
    *type = cGetVirtualAccessibleNamePackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getVirtualAccessibleName(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
            wcsncpy(name, pkg->rName, max);
            PrintDebugString("\r\n    WinAccessBridge::getVirtualAccessibleName: Virtual name = %ls", name);
            return TRUE;
        }
    }
    return FALSE;
...
WinAccessBridge::requestFocus(long vmID, AccessibleContext accessibleContext) {
    ...
    *type = cRequestFocusPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::requestFocus(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::selectTextRange(long vmID, AccessibleContext accessibleContext, int startIndex, int endIndex) {
    ...
    *type = cSelectTextRangePackage;
    ...
    PrintDebugString("\r\n    WinAccessBridge::selectTextRange(%X, %X %d %d)", vmID, accessibleContext,
                     startIndex, endIndex);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
...
WinAccessBridge::getTextAttributesInRange(long vmID, AccessibleContext accessibleContext, 
    ...
    *type = cGetTextAttributesInRangePackage;
    ...
    PrintDebugString("\r\n    WinAccessBridge::getTextAttributesInRange(%X, %X %d %d)", vmID, accessibleContext,
                     startIndex, endIndex);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
...
WinAccessBridge::getVisibleChildrenCount(long vmID, AccessibleContext accessibleContext) {
    ...
    *type = cGetVisibleChildrenCountPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getVisibleChildrenCount(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getVisibleChildren(long vmID, AccessibleContext accessibleContext, int startIndex,
    ...
    *type = cGetVisibleChildrenPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getVisibleChildren(%X, %X)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::setCaretPosition(long vmID, AccessibleContext accessibleContext, int position) {
    ...
    *type = cSetCaretPositionPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::setCaretPosition(%X, %X %ls)", vmID, accessibleContext);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextInfo(long vmID,
    ...
    *type = cGetAccessibleTextInfoPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextInfo(%X, %X, %X, %d, %d)", vmID, AccessibleContext, textInfo, x, y);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextItems(long vmID, 
    ...
    *type = cGetAccessibleTextItemsPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextItems(%X, %X, %X, %d)", vmID, AccessibleContext, textItems, index);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextSelectionInfo(long vmID,
    ...
    *type = cGetAccessibleTextSelectionInfoPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextSelectionInfo(%X, %X, %X)", vmID, AccessibleContext, selectionInfo);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextAttributes(long vmID,
    ...
    *type = cGetAccessibleTextAttributeInfoPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextAttributes(%X, %X, %d, %X)", vmID, AccessibleContext, index, attributes);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextRect(long vmID,
    ...
    *type = cGetAccessibleTextRectInfoPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextRect(%X, %X, %X, %d)", vmID, AccessibleContext, rectInfo, index);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getCaretLocation(long vmID,
    ...
    *type = cGetCaretLocationPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getCaretLocation(%X, %X, %X, %d)", vmID, AccessibleContext, rectInfo, index);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextLineBounds(long vmID,
    ...
    *type = cGetAccessibleTextLineBoundsPackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextLineBounds(%X, %X, %d, )", vmID, AccessibleContext, index);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::getAccessibleTextRange(long vmID,
    ...
    *type = cGetAccessibleTextRangePackage;
    ...
    PrintDebugString("\r\nWinAccessBridge::getAccessibleTextRange(%X, %X, %d, %d, )", vmID, AccessibleContext, start, end);
    // need to call only the HWND/VM that contains this AC
    HWND destABWindow = javaVMs->findAccessBridgeWindow(vmID);
    if (destABWindow != (HWND) 0) {
        if (sendMemoryPackage(buffer, sizeof(buffer), destABWindow) == TRUE) {
...
WinAccessBridge::addAccessibleSelectionFromContext(long vmID,
                                                   jobject AccessibleContext, int i) {
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
...
WinAccessBridge::addJavaEventNotification(jlong type) {
    PrintDebugString("\r\nWinAccessBridge::addJavaEventNotification(%X)", type);
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
    ...
    *pkgType = cAddJavaEventNotificationPackage;
    pkg->type = type;
    pkg->DLLwindow = dialogWindow;

    PrintDebugString("  ->pkgType = , eventType = , DLLwindow = ", 
...
WinAccessBridge::removeJavaEventNotification(jlong type) {
    PrintDebugString("\r\nin WinAccessBridge::removeJavaEventNotification(%X)", type);
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
...
WinAccessBridge::addAccessibilityEventNotification(jlong type) {
    PrintDebugString("\r\nin WinAccessBridge::addAccessibilityEventNotification(%X)", type);
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
...
WinAccessBridge::removeAccessibilityEventNotification(jlong type) {
    PrintDebugString("\r\nin WinAccessBridge::removeAccessibilityEventNotification(%X)", type);
    if ((AccessBridgeJavaVMInstance *) 0 == javaVMs) {
        return;
    }
