...
#define LOAD_FP(result, type, name) \
     ...
     */
	__declspec(dllexport) BOOL getAccessibleTableCellInfo(long vmID, AccessibleTable accessibleTable,
	...
	__declspec(dllexport) jint getAccessibleActions(long vmID, AccessibleContext accessibleContext) {
        if (theAccessBridgeInitializedFlag == TRUE) {
            BOOL ret = theAccessBridge.getAccessibleActions(vmID, accessibleContext, &actions);
			if (ret == TRUE)
	...
	__declspec(dllexport) BOOL getAccessibleActionItem(jint index, wchar_t* action, jint len) {
		if (index < 0 || index >= actions.actionsCount)
			return FALSE;
		lstrcpyn(action, actions.actionInfo[index].name, len);
		return TRUE;
	}
     ...
     */
	__declspec(dllexport) BOOL selectTextRange(const long vmID, const AccessibleContext accessibleContext,
     ...
     */
	__declspec(dllexport) BOOL getCaretLocation(long vmID, AccessibleContext ac, AccessibleTextRectInfo *rectInfo, jint index) {
        if (theAccessBridgeInitializedFlag == TRUE) {
